...
Use short, imperative commit subjects like `Add Cerebellum recovery guidance for retries`. Release commits may use the existing versioned style, for example `v26.330.2: Browser progress ledger + task checkpoints`. PRs should explain the user-visible change, note risky areas, list validation commands, and include screenshots or terminal captures for CLI/TUI changes.
